...
- Python 3.9+
- [Claude Code CLI](https://docs.claude.com/claude-code) 2.1+（已验证 2.1.105）
...
onboarding 完成后，试几个命令感受一下：

```
「站会」        → 秘书汇报你最近做了什么
「导入历史」    → 把之前的 Claude Code 对话导入 wiki
「关系图」      → 生成知识关系图，浏览器打开看看
```

用几天之后再试 **「周会」** 和 **「复盘 X」**，wiki 有内容了效果更明显。
